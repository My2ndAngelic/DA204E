        ...
        public class Arrays
        {
            ...
            public static string DeepToString<T>(T[,] array)
            {
                ...
                for (int i = 0; i < array.GetLength(0); i++)
                {
                    ...
                    {
                        result.Append($"{(array[i, j] == string.Empty ? "string.Empty" : array[i, j])}, "); // Add the value and a comma
                    }
                    result.Remove(result.Length - 2, 2).Append($"], "); // Remove the last comma and space and add the closing bracket
                }
                result.Remove(result.Length - 2, 2).Append(']'); // Remove the last comma and space and add the closing bracket
            ...
            public static string ToString<T>(T[] array)
            {
                ...
                for (int i = 0; i < array.Length; i++)
                {
                    result.Append($"{(array[i] == string.Empty ? "string.Empty" : array[i])}, "); 
                }
                result.Remove(result.Length - 2, 2).Append(']'); // Remove the last comma and space and add the closing bracket
                return result.ToString(); // Return the string
            }
